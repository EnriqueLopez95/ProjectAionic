...
export class FirebaseService {
  ...
  // Método para obtener el usuario actual y su rol desde Firestore
  getCurrentUserWithRole(): Observable<User> {
    return this.auth.authState.pipe(
      switchMap(user => {
        if (user) {
          return this.firestore.doc<User>(`users/${user.uid}`).valueChanges(); // Obtiene cambios en el documento del usuario actual
        } else {
